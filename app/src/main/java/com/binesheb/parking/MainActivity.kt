...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUi() { val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(16, 10, 16, 10) }; val header = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }; val title = TextView(this).apply { text = "PARKING • ${role.label}"; textSize = 20f; setPadding(4, 4, 12, 4) }; val roleButton = Button(this).apply { text = "Role"; setOnClickListener { chooseRole() } }; val settingsButton = Button(this).apply { text = "Server"; setOnClickListener { configureServer() } }; header.addView(title, LinearLayout.LayoutParams(0, 60, 1f)); header.addView(roleButton); header.addView(settingsButton); preview = PreviewView(this); result = TextView(this).apply { text = "Waiting for vehicle…"; textSize = 30f; setPadding(8, 12, 8, 12) }; status = TextView(this).apply { textSize = 15f }; root.addView(header); root.addView(preview, LinearLayout.LayoutParams(-1, 0, 1f)); root.addView(result); root.addView(status); setContentView(root) }
    ...
    private fun onCandidate(plate: String) { if (plate == lastPlate) stableCount++ else { lastPlate = plate; stableCount = 1 }; if (stableCount < 3 || System.currentTimeMillis() - lastEventAt < 4000) return; lastEventAt = System.currentTimeMillis(); stableCount = 0; val event = store.insert(ParkingEvent(plate = plate, eventType = role.eventType, timestamp = System.currentTimeMillis(), deviceRole = role.code)); result.post { result.text = "${event.plate}  •  ${event.eventType}" }; status.post { status.text = "Recorded locally • ${store.count()} events • sync queue active" }; sync.enqueue(event) }
